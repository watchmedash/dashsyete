...
Dash City v3 — a multiplayer on-foot **blaster deathmatch** in the browser (long-term goal: battle royale; rounds/zone come later). Players are Kenney blocky characters running through **one dense mainland city** (~288 m): ring road + avenues + central roundabout, skyscraper downtown, industrial harbor strip with a dock, west park, east commercial block, a graveyard corner, and suburbs. Free-for-all: foam-dart blasters (flat damage per weapon), grenade pickups, knockouts score on a personal leaderboard, respawns, endless drop-in session. Cars are **parked static decor** (cover) — there is NO driving. Desktop (WASD + mouse aim, pointer lock) and mobile (joystick + fire/jump buttons + swipe aim). V cycles camera: 3rd-back / 1st-person / 3rd-front (crosshair hides in 3rd-front).
...
- `npm run assets` — copy GLBs from the `kenney_*` packs into `client/public/assets/` (run once after clone; packs + copied assets are git-ignored)
...
- Hazard respawn: falling into the sea (or 🆘 unstuck, 5 s cooldown) teleports to the NEAREST clear road tile. Knockout respawns pick the clear spawn point farthest from living enemies.
- The map (`shared/src/cityMap.ts`, `buildCityMap()`) is deterministic and drives BOTH client visuals and server colliders — change geometry there, never in one side only. Road tiles pick model+rotation from a neighbour-based classifier; rotation tables were MEASURED (don't re-guess). Ground is per-landmass slabs; the sea has no floor. `parkedCars` get static colliders via `parkedCarCollider()`.
- Building/prop colliders come from `modelFootprints.ts` (measured — `scripts/measure-glb.mjs`; never guess; models place by bbox CENTER). `fitsTile()` filters building lists; keep `probe-road-blockers` clean.
- Every visible decor object reacts: sturdy pieces have static colliders; organic bits (hay, pumpkins, cones, boxes) are knockable dynamic props. Props are mirrored inside client prediction and RENDERED from there (interp-rendered props visually lag inside the predicting own character).
