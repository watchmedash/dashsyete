...
Free-for-all in **12-slot cube ROOMS** (`server/src/rooms.ts`, user spec 2026-08-13): one server process hosts many independent cubes; new players always join a populated cube with space, a fresh cube spins up only when all are full of humans; bots backfill every unused slot (solo = vs 11 bots); a cube whose last human leaves dissolves; one warm empty cube is always kept ready. The leaderboard is HOME-MENU ONLY (`/api/leaderboard`, global across cubes, pure knockouts) — the in-match Tab leaderboard was removed. `Game.start(port)` remains a single-room server for tests. Foam-dart blasters with FINITE ammo, **single gun slot** (4-cell hotbar: 1 gun / 2 destroy tool / 3 grenades / 4 blocks; wheel or digit keys or tap to select), distance damage falloff (snipers flat), 2× headshots, +50 HP on kill, NO natural regen. Minecraft-style timed mining (crack decal), block placing (build cap alt > 30 rejected), water that flows/swims/hides-you/drowns-you slowly, grenades that only explode after ground contact (lit landing marker, ~26 m max-power lob), knockable crates→boxless floating pickups with color-coded beacons, gun drops on pickup swap, per-face block-debt regeneration. Desktop (WASD + pointer lock, Tab leaderboard, U unstuck, RMB scope, V camera cycle, Esc pause/settings) and mobile (joystick + swipe aim + fire/jump/nade/zoom buttons, tappable hotbar). Settings (menu + Esc): sensitivity, volume, FOV — persisted.
